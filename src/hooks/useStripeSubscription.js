...
const API_BASE_URL = 'https://api-voice-staging.botnoi.ai/api/stripe/stripe_subscription';
...
export function useStripeSubscription({
  ...
  const checkout = useCallback(async (packageId, options = {}) => {
    if (!packageId) {
    ...
    try {
      ...
      const durationMonths = Number(options?.durationMonths ?? 0);
      const expectedTotalUsd = Number(options?.expectedTotalUsd ?? 0);

      if (Number.isFinite(quantity) && quantity > 1) {
        query.set('quantity', String(Math.floor(quantity)));
      }
      if (Number.isFinite(durationMonths) && durationMonths > 0) {
        query.set('duration_months', String(Math.floor(durationMonths)));
      }
