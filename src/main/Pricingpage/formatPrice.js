...
/**
 * Returns { display, currency } for showing price in USD or Kyats based on language.
...
export function formatPriceDisplay(price, language, t) {
  ...
  if (language === 'my') {
    const kyats = Math.round(num * USD_TO_KYATS);
    return { display: kyats.toLocaleString(), currency: t('pricing.currencyKyats') };
  }
